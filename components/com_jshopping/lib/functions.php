...
function getBuildFilterListProduct($contextfilter, $no_filter = array()){
    ...
    if ($jshopConfig->admin_show_product_extra_field){
		...
		$extra_fields_t = $mainframe->getUserStateFromRequest($contextfilter.'extra_fields_t', 'extra_fields_t', array());
...
static $load;
    if (!JRequest::getInt('no_lang') && !$load){

...
$document->addStyleSheet(JUri::root() . 'media/com_ucs_users/css/list.css');

$user      = JFactory::getUser();
$userId    = $user->get('id');
$listOrder = $this->state->get('list.ordering');
$listDirn  = $this->state->get('list.direction');
$canOrder  = $user->authorise('core.edit.state', 'com_ucs_users');
$saveOrder = $listOrder == 'a.`ordering`';

if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_ucs_users&task=users.saveOrderAjax&tmpl=component';
...
<form action="<?php echo JRoute::_('index.php?option=com_ucs_users&view=users'); ?>" method="post"
	...
	<?php echo JHtml::_('jgrid.published', $item->state, $i, 'users.', $canChange, 'cb'); ?>
				...
				<?php endif; ?>
				<?php if ($canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_ucs_users&task=user.edit&id='.(int) $item->id); ?>">
